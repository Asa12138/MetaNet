...
#'
#' @param p a igraph or metanet object
  ...
  suppressMessages(anno_edge(go,nodeGroup)%>%get_e()->edge)
{
  ...
  suppressWarnings({V(tmp_go)$count=tmp_v%>%dplyr::group_by_(Group)%>%dplyr::count()%>%dplyr::pull(n)})
...
skeleton_plot<-function(ske_net,...) {
  ...
  for (i in unique(tmp_e$e_type)) {
    ...
    c_net_plot(tmp_go1,vertex.size=pcutils::mmscale(V(tmp_go1)$size,10,16),
...
#statistics
  ...
  dplyr::group_by(bb,from)%>%dplyr::summarise(n=sum(count))%>%
    dplyr::arrange(-n)%>%dplyr::top_n(topN,n)%>%dplyr::pull(from)->nnn
  dplyr::filter(edge,v_class_from%in%nnn,v_class_to%in%nnn)%>%dplyr::pull(id)->eid
  subgraph.edges(go,eid)->go1
  get_v(go1)->tmp_v
  if(is.null(colors))colors=setNames(unique(tmp_v$color),unique(tmp_v$v_class))
  else tmp_v$color=condance(data.frame(tmp_v$color,tidai(tmp_v$v_class,colors)))
#plot
  dplyr::filter(bb,from%in%nnn,to%in%nnn)%>%pcutils::my_circo(.,reorder = FALSE, pal= colors)
#legend
...
rand_net<-function(go = go){
  ...
  data1 = data.frame(freq= igraph::degree_distribution(go),net = "Network",
                     degree = 0:(length(degree_distribution(go))-1))

  data2 = data.frame(freq= igraph::degree_distribution(rand.g) ,net = "Random E-R",
