...
#'
#' @description this is just a random generation method, the module number of result is not exactly the module_number, you can change the inter_modu_density and intra_modu_density to get the proper result.
...
modu_net <- function(module_number = 3, n_node_in_module = 30,
    ...
    for (i in seq_len(module_number)) {
        seq <- seqls[[i]]
        mat[seq, seq] <- as.matrix(igraph::get.adjacency(igraph::erdos.renyi.game(n_node_in_module[i], intra_modu_density)))
    }
...
modu_dect <- function(go, method = "cluster_fast_greedy", n_node_in_module = 0, delete = FALSE) {
...
#' @examples
#' data("c_net")
#' modu_dect(co_net, n_node_in_module = 30) -> co_net_modu
...
zp_analyse <- function(go_m, mode = 2, use_origin = TRUE) {
    ...
    if (use_origin) {
        {
            go1 <- anno_vertex(go1, data.frame(row.names = V(go1)$name, module = V(go1)$original_module %>% as.numeric()))
        } %>% suppressMessages()
    } else {
